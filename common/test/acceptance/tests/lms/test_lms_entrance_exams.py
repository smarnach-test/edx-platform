...
class EntranceExamTest(UniqueCourseTest):
    ...
    def setUp(self):
        ...
        ee_subsection = None
        outline = course_fixture.get_course_outline()
        for child in outline['child_info']['children']:
            if child.get('display_name') == "Entrance Exam":
                ee_subsection = child['child_info']['children'][0]

        if ee_subsection:
            course_fixture.create_xblock(ee_subsection['id'], problem)
...
class EntranceExamPassTest(EntranceExamTest):
    ...
    def test_course_is_unblocked_as_soon_as_student_passes_entrance_exam(self):
        ...
        self.assertTrue(self.courseware_page.is_not_passed_message_present)
        problem_page.click_choice('choice_1')
        problem_page.click_check()
        self.courseware_page.wait_for_page()
        self.assertFalse(self.courseware_page.is_not_passed_message_present)
        self.assertTrue(self.courseware_page.is_passed_message_present)
