...
def issued_certificates_features(course_key, features):
    """
    Return list of issued certificates features as dictionaries against the given course key.

    issued_certificates_features(course_key, features)
    would return [
    ...
    generated_certificates = list(GeneratedCertificate.objects.filter(course_id=course_key, status='downloadable')
